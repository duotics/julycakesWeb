...
$sdateN = strtotime ( '-45 day' , strtotime ( $sdate ) ) ;
$sdateN = date ( 'Y-m-j' , $sdateN );
$query_RSvi = sprintf("SELECT db_items.item_id AS i_id, db_items.item_cod AS i_cod, db_items.item_aliasurl AS i_url, db_items.item_img AS i_img, db_items.item_date as i_date, db_items.item_status as i_stat, db_items_brands.name as b_nom, db_items_brands.URL as b_url, db_items_brands.img AS b_img, db_items_type.typUrl AS c_url 
FROM db_items 
...
WHERE db_items.item_status=1 AND item_date>=%s ".$qbrand." GROUP BY db_items.item_id %s LIMIT 300", 
SSQL($sdateN,'date'),
SSQL($qsort,''));
...
<?php echo genBrdc(NULL,NULL,'Latest Releases') ?>
<?php if($vTit==TRUE){ ?>
<div id="dcat_head">
<div id="dcat_tit">
...
<script>
	$(document).ready(function() {
		$('#selBrand').change(function(){
			var url=RAIZ+'LatestReleases/'+$('#selBrand').val()+'/'+$('#selSort').val();
			location.href=url;
		});
