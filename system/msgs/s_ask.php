    ...
    <tr>
        ...
        <div style="margin:5px; background:#FFF; padding:7px; text-align:center; border-right: 5px solid #ddd;">
			<a href="<?php echo $detRF['link'] ?>" style="text-decoration:none; color:#2b2b2b" target="_blank">
           	<div>
				<h1 style="font-weight: normal"><?php echo $detRF['tit'] ?></h1>
                <h2 style="font-size:90%; color:#666"><?php echo $detRF['cod'] ?></h2>
            </div>
            <div>
            	<img src="<?php echo $detRFi['t'] ?>" alt="<?php echo $detRF['tit'] ?>" style="width:inherit;max-width:100%;height:auto;"/>
            </div>
            <div style="margin-top:5px; font-size:75%; background:#eee; color:#333; padding:5px 0; border:1px solid #ddd;"><?php echo $detRF['link'] ?></div>
        	</a>
        </div>
		</td>
        <?php }else{ $valColSpan=2; } ?>
        <td width="70%" colspan="<?php echo $valColSpan ?>" style="padding:0; width: 70%">
        <table style="width:100%" cellpadding="4" cellspacing="1">
        
        <?php if(($detC['name'])||($detC['company'])){ ?>
        <tr style="font-size:125%;">
            <td style="width:25%; background:#ccc; color:#666; text-align: right">Name / Company</td>
            <td style="width:75%" bgcolor="#FFFFFF"><strong><?php echo $detC['name']; ?></strong>. <?php echo $detC['company']; ?></td>
        </tr><?php } ?>
        <?php if($detM['mail']){ ?>
        <tr style="font-size:125%;">
            <td style="background:#ccc; color:#666; text-align: right">E-Mail</td>
        ...
        <tr>
            <td style="text-align: right">Referred</td>
